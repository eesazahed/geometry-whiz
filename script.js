...
let surfaceAreaOfCylinder = (radius, height) => pi * square(radius) * 2 + 2 * pi * radius * height;

// cones
let volumeOfCone = (radius, height) => (1 / 3) * pi * square(radius) * height;
let surfaceAreaOfCone = (radius, slantHeight) => pi * radius * (radius + Math.sqrt(square(slantHeight) + square(radius)));

// length of line
let lengthOfLine = (x1, x2, y1, y2) => Math.sqrt(square(x2 - x1) + square(y2 - y1));
let midpoint = (x1, x2, y1, y2) => [(x1 + x2) / 2, (y1 + y2) / 2];
let hypotenuse = (a, b) => Math.sqrt(square(a) + square(b));

// slope
let slope = (x1, x2, y1, y2) => (y2 - y1) / (x2 - x1);
